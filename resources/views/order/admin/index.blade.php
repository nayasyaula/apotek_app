...
@section('content')
    <div class="mt-5">
        <div class="justify-content-start d-flex">
            <form action="{{ route('order.searchAdmin')}}" method="GET" >
            <input type="date" name="searchAdmin" id="searchAdmin" value="{{ old('searchAdmin')}}">
           <button type="submit" class="btn btn-primary">Cari Data</button>
           <a href="{{ route('admin.order.data')}}" class="btn btn-primary">Reset</a>
            </form>
        </div>
        ...
        {{-- current page : ambil posisi ada di page ke berapa - 1 ( misal uda klik next lagi ada dipage 2 berarti jadi 2-1 = 1),
            ...
            {{-- 1. nama obat Rp.1000  --}}
                <li>{{ $medicine['name_medicine'] }} <small>Rp. {{ number_format($medicine['price'], 0, ',', '.') }} 
                    <b>(qty : {{ $medicine['qyt'] }}) </b></small> 
                    = Rp. {{ number_format($medicine['sub_price'], 0, ',', '.') }} </li>
            @endforeach
                </ol>
            </td>
            @php 
